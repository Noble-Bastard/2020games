class KeybordListener:
    ...
    def replykeyboardChecker(self, message):
        if message.chat.type == 'private':
            if message.text == self.config.MAINKEYBOARD['top-ten']:
                topGames = self.getTopGames.getNames()
                markup = self.manager.viewTopGames(topGames)
                self.bot.send_message(message.chat.id, 'Лучшие игры за год:', reply_markup=markup)
                ...
                self.bot.send_message(
                    message.chat.id, 'Команда не найдена.')
            ...
            if call.message:
                if call.data in self.topGames:
                    ...
                    self.bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                               text=info, parse_mode="HTML", reply_markup=None)

                    self.bot.answer_callback_query(callback_query_id=call.id, show_alert=False, text="Success!")
